...
1. **Domain boost** (structural) — `memory.search.domainBoosts` is an array of `{pattern, boost, semantics?}` entries. For each result, the max-matching substring rule multiplies the combined score. Default: uniform 3.0 for `/identity/`, `/memory/MEMORY.md`, `/user/`, `/crm/`. Rationale: foundational files are rarely retrieved so use-dependent boosting has nothing to work with; structural boost compensates for cold-start.

   **Optional embedding-similarity gating**: each entry can include a `semantics` descriptor (short phrase naming the query shapes that should trigger the boost). At search time, the query embedding is compared against each semantics embedding; the boost fires only when cosine similarity ≥ `domainBoostMinCosine`. Default threshold 0 (gating OFF — all rules fire, legacy behavior). Raising it to 0.30-0.35 filters user-preference boosts off technical queries but on the 112-case harness this drops F1 by ~0.02 because the harness mix favors user/identity queries. Ship off; flip up per-query via the reload endpoint when a specific technical query is losing to crowding.
