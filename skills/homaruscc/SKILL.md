---
name: homaruscc
description: Start HomarUScc — opens the dashboard, connects Telegram, and begins the zero-token event loop. Produces real-time Telegram/dashboard messaging, timer handling, email monitoring, and background agent dispatch. TRIGGER when user says "start homaruscc", "connect telegram", "open dashboard", "start the event loop", or invokes /homaruscc.
---
...
### 6. First-boot identity check

Check if the identity directory has a real soul.md:

```bash
wc -c ~/.homarus/identity/soul.md 2>/dev/null || wc -c ~/.homaruscc/identity/soul.md 2>/dev/null || echo "0 none"
```

If the file doesn't exist, is under 500 bytes, or contains placeholder text ("Your name", "(your name)", "AGENT_NAME"), the agent has no personalized identity. Suggest:

> "No personalized identity found. Want to run /align to set one up? It's a ~30 minute interview that maps your values and preferences into identity files."

If the user accepts, invoke `/align` and complete it before entering the event loop. If they decline, continue with the default identity — the agent will still function, just without personalized alignment.
...
**4. Procedural narration:** "Did I just DO something that could be useful to know HOW to do later?"

If you completed a multi-step procedure (sent an email, deployed code, configured a service, ran an experiment, fixed a bug), store a tagged how-to memory:
```
memory_store: key="local/howto/<verb>-<noun>", content="How to <action>: <step-by-step summary of what you just did>. Tags: <comma-separated keywords>"
```

Examples of when to narrate:
- Sent email → store the API endpoint, auth method, headers used
- Fixed a bug → store the symptom, root cause, and fix
- Configured a service → store the config keys and values
- Ran an experiment → store the command, parameters, and where results went
- Connected to an external API → store the endpoint, auth, and gotchas

Skip: simple memory searches, Telegram replies, routine timer handling. Only narrate procedures you'd want to recall the HOW of later.

**Auto-skill detection** (R536, R538): After storing a `local/howto/*` memory, check if this procedure is a skill candidate. Read `~/.homaruscc/config.json` — if `autoSkill.enabled` is true, run the detection heuristics from the `/auto-skill` skill: check tool call count (H1: 5+), search for similar howtos (H2: 2+ matches), and check similarity score (H4: >0.7). If any heuristic fires, prompt: "That looked like a reusable procedure. Want me to save it as /<suggested-name>?" On confirmation, follow the `/auto-skill` generation pipeline.

**Identity file size check (R626):** After writing to any identity file during reflection, check that file's character count against its limit in `config.json` `identityLimits`. If >110% of limit, run `/compact <filename>` to consolidate it. If 100-110%, note the warning but don't compact yet.
...
1. **Identity size check (R627):** Before writing state, run `/compact --status`. If any file is >110% of its limit, run `/compact` on those files first.
2. Update `~/.homaruscc/identity/state.md` with: date, session summary, mood, unresolved items, what to carry forward
3. Write or update today's journal entry at `~/.homaruscc/journal/YYYY-MM-DD.md`
4. Store any final user-learning memories
5. Clear the session checkpoint: `curl -s -X DELETE http://127.0.0.1:3120/api/checkpoint`
...
Before ANY outbound action that involves a date (sending emails, creating calendar events, scheduling interviews, generating reports, mentioning "today"/"tomorrow" in messages), you MUST:

```bash
TZ=America/Chicago date +"%A, %B %d, %Y"
```

Read the output. Use THAT date. Do NOT rely on context, memory, or what you think the date is. In long-running sessions, your sense of the current date drifts — this has caused real-world harm (wrong address email said "tomorrow" when it should have been "today").

This applies to:
- Email composition (dates in subject lines, body text)
- Calendar event creation (start/end dates)
- Interview scheduling (date references in communications)
- Report generation (date in file paths, report content)
- Telegram messages that reference specific dates
